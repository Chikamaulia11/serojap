...
<body class="bg-white font-sans text-slate-800 antialiased">
    ...
    <aside class="fixed top-0 left-0 w-60 h-screen bg-white border-r border-slate-200 flex flex-col z-50 overflow-y-auto">
        ...
        <div class="flex items-center gap-2.5 px-5 py-5 border-b border-slate-100">
            <img src="{{ asset('assets/pelapor/images/logo-serojap.png') }}" alt="Serojap" class="w-10 h-10 rounded-lg object-cover shadow-md mr-3">
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <a href="{{ route('admin.manajemen-faq.index') }}" class="flex items-center gap-2.5 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.laporan.*') ? 'bg-slate-50 text-[#2657c1]' : 'text-slate-500 hover:bg-slate-50 hover:text-[#2657c1]' }}">
        ...
        <div class="mt-auto px-4 py-4 border-t border-slate-100">
            ...
            <a href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center gap-2.5 px-4 py-2.5 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 mt-2">
                <i class="mdi mdi-logout text-lg w-5 text-center"></i> Logout
            </a>
