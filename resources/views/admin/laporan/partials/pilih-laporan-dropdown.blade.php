...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm">
    ...
    <div class="px-6 py-4">
        <form method="GET" action="{{ route('admin.laporan.show', ['id' => $laporanSaatIni->id_laporan ?? 0]) }}" class="w-full">

            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Pilih Laporan</label>
<select name="id"
                    onchange="this.form.action='{{ route('admin.laporan.show', ['id' => '__ID__']) }}'.replace('__ID__', this.value); this.form.submit();"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5">
            @foreach($daftarLaporan as $l)
                <option value="{{ $l->id_laporan }}" {{ (isset($laporanSaatIni) && (int) $laporanSaatIni->id_laporan === (int) $l->id_laporan) ? 'selected' : '' }}>
